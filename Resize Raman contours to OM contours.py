#Script is used to scale the nanosheet from produced raman maps to the same aspect ratio as the corresponding optical image so
...
for filename in glob.glob('D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\Raman Threshold/*.png'):
    #trim sample/file name from filepath and remove .txt extension
    head, tail = os.path.split(filename)
    image_name = tail[:-4]
    ...
    raman_map = cv2.imread(f"D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\Raman area maps/{image_name}.png") #load raman area map
   
    raman_roi = raman_map[y:y + h, x:x + w]  #crop roi from map
    
    cv2.imwrite(f"D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\Raman ROI/{image_name}.tif", raman_roi) #save raman roi 
...
for filename in glob.glob('D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\OM Raman/*.tif'):
    om_img = cv2.imread(filename) #read image
    roi, image_name = get_OM_shape(filename) #get roi and image name
    cv2.imwrite(f"D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\OM resave/{image_name}.tif", om_img) #save the om_image to prevent resizing errors when loading/reloading images with cv2, cause unknown to me but it appears the image dpi changes but size etc stays the same.

    height, width = get_OM_dimensions(roi) #get height and width of roi
    dimensions = (width, height) #reorder to use in cv2.resize
    
    raman_roi = cv2.imread(f"D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\Raman ROI/{image_name}.tif") #load raman roi
     
    raman_rescale = cv2.resize(raman_roi, dimensions, interpolation= cv2.INTER_AREA) #rescale to aspect ratio of OM image
    cv2.imwrite(f"D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\Rescaled raman/{image_name}.tif", raman_rescale) 
    ...
    cv2.imwrite(f"D:\Anticounterfeiting Paper\POC Study\Flake Library\Raman Shapes\Rescaled Raman Pad/{image_name}.tif", raman_pad) #save rescale raman image